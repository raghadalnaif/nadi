...
export function MemberLoginForm() {
  const [phoneState, sendAction, sending] = useActionState(
    sendCode,
    { step: "phone" } as LoginState
  );

  if (phoneState.step === "code") {
    return <CodeStep initial={phoneState} />;
  }
  ...
  return (
    <form action={sendAction} className="space-y-4">
      ...
      <p className="text-xs text-slate-400 text-center leading-relaxed">
        سيصلك رمز مكوّن من ٦ أرقام على جوالك. لا تحتاج كلمة مرور.
      </p>
    </form>
  );
}

function CodeStep({ initial }: { initial: Extract<LoginState, { step: "code" }> }) {
  const [state, verifyAction, verifying] = useActionState(checkCode, initial as LoginState);
    ...
    <form action={verifyAction} className="space-y-4">
      ...
      <div className="rounded-xl bg-emerald-50 ring-1 ring-emerald-100 px-4 py-3 flex items-start gap-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-emerald-700 mt-0.5" dir="ltr">
            {current.phone}
          </p>
        </div>
      </div>

      {/* في التطوير فقط — يسهّل الاختبار بلا إرسال فعلي */}
      {current.devCode && (
